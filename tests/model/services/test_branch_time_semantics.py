...
class BranchTimeSemanticsTests(unittest.TestCase):
    ...
    def test_branch_events_are_recorded_in_history_without_consuming_time(self):
        """分支不计时，但挂起时仍记入 event_history，冷却/次数生效。"""
        agent = make_agent(money=100)
        self.play_turn.handle_player_text(agent, self.world, "看人参")
        self.play_turn.handle_player_text(agent, self.world, "买")

        self.assertEqual(agent.event_history.trigger_count("branch"), 1)

    def test_unaffordable_branch_is_rejected_without_going_into_debt(self):
        agent = make_agent(money=3)
        self.play_turn.handle_player_text(agent, self.world, "看人参")
        result = self.play_turn.handle_player_text(agent, self.world, "买")
        self.assertEqual(result.reject_reason, "你囊中羞涩，买不起。")
        self.assertEqual(agent.money, 3)
        self.assertEqual(agent.pending_encounter_id, "branch")
